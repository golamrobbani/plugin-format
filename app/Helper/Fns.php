...
class Fns
{
    ...
    {
        $white_label = get_option("grpf_white_label");
        $logo = null;
    ...
    public static function locate_template($name)
    {
        ...
        $template[] = grpf()->get_template_path() . $name . ".php";

        if (!$template_file = locate_template(apply_filters('grpf_locate_template_names', $template))) {
            $template_file = GRPF_PATH . "templates/$name.php";
        }

        return apply_filters('grpf_locate_template', $template_file, $name);
    }
...
<h3>Plugin Format</h3>";
        $email_footer = get_option('grpf_email_footer');
        if (grpf()->wage()) {
        ...
        $social_list = [
            [
                'icon' => grpf()->get_asset_uri('img/email/') . 'wp.png',
                'url' => 'https://wordpress.org/plugins/plugin-format',
            ],
            [
                'icon' => grpf()->get_asset_uri('img/email/') . 'facebook.png',
                'url' => 'https://www.facebook.com/plugin-format',
            ],
            [
                'icon' => grpf()->get_asset_uri('img/email/') . 'twitter.png',
     ...
     *
     * GRPF_TEMPLATE_DEBUG_MODE will prevent overrides in themes from taking priority.
     *
    ...
    public static function get_template_part($slug, $args = null, $include = true)
    {
        ...
        $template = GRPF_TEMPLATE_DEBUG_MODE ? '' : locate_template(
            array(
                "{$slug}.php",
                grpf()->get_template_path() . "{$slug}.php"
            )
        );

        // load template from pro plugin if exist
        if (!$template && function_exists('grpfp')) {
            $fallback = grpf()->plugin_path() . "-pro" . "/templates/{$slug}.php";
            $template = file_exists($fallback) ? $fallback : '';
        }

        // load template from current plugin if exist
        if (!$template) {
            $fallback = grpf()->plugin_path() . "/templates/{$slug}.php";
            $template = file_exists($fallback) ? $fallback : '';
        }

        // Allow 3rd party plugins to filter template file from their plugin.
        $template = apply_filters('grpf_get_template_part', $template, $slug);
    ...
    public static function get_template($fileName, $args = null)
    {
        ...
        if (!file_exists($located)) {
            ...
            self::doing_it_wrong(__FUNCTION__, sprintf(__('%s does not exist.', 'plugin-format'), '<code>' . $located . '</code>'), '1.0');

            return;
        }

        // Allow 3rd party plugin filter template file from their plugin.
        $located = apply_filters('grpf_get_template', $located, $fileName, $args);

        do_action('grpf_before_template_part', $fileName, $located, $args);

        include $located;

        do_action('grpf_after_template_part', $fileName, $located, $args);
    }
